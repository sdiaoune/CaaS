...
export default function PlaygroundPage() {
  ...
  const handleRunQuery = async () => {
    ...
    try {
      ...
      const res = await fetch('/app/api/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId: project?.id, query, k: 6 }) })
      const data = await res.json()
      const t1 = performance.now()
      const chunks: RetrievedChunk[] = (data.hits || []).map((h: any, i: number) => ({ id: String(h.chunk_id), text: h.content, source: h.document?.title || `Doc ${i+1}`, score: h.score, tokens: Math.round((h.content?.length || 0)/4) }))
      const answer = chunks.length ? `Based on ${chunks.length} contexts, here is a synthesized answer.` : 'No context found.'
      setResponse({ answer, chunks, latency: Math.round(t1 - t0), tokens: chunks.reduce((a,c)=>a+c.tokens,0), cost: 0, trace: { retrieval: { duration: Math.round(t1 - t0), count: chunks.length }, reranking: { duration: 0, scores: chunks.map(c=>c.score) }, guardrails: { duration: 0, decisions: [] }, generation: { duration: 0, tokens: 0 } } })
    } finally {
      setIsLoading(false)
    }
  }
